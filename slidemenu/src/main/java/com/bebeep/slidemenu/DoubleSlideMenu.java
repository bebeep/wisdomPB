...
//                slideMenu.layout(0,0,mainMenu.getMeasuredWidth(), mainMenu.getMeasuredHeight());
        ...
        public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
            if(menulocation == DragState.LEFT){
                if(mainMenu.getLeft() < dragRange /5) close(); //在拖拽范围的左边->关闭
                else if(mainMenu.getLeft() >= dragRange /5) open(); //在拖拽范围的右边->打开
                if(xvel > 300 && currentState!= DragState.STATE_OPEN &Math.abs(yvel) < 300){
                    open();
                }else if(xvel< -300 && currentState != DragState.STATE_CLOSE){
                    close();
                }
            }else if(menulocation == DragState.RIGHT){
